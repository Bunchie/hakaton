...
let iteration = {};
numbers.forEach(function(number, indexNumber) {
	processor.forEach(function(proc, index) {
		if(iteration[index] === indexNumber) { boolProcessor[index] = false; }
		if(number % 13 === 0) {
			if(!boolProcessor[index]) {
				boolProcessor[index] = true;
				indexResult.push(index);
				iteration[index] = number/13;
			}
			//use first proc
		}
		if(number % 2 === 0) {
